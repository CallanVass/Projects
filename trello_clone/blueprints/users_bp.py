...
def login():
    ...
    if user and bcrypt.check_password_hash(user.password, user_info["password"]):
        ...
        token = create_access_token(identity=user.id, expires_delta=timedelta(hours=2)) # ,additional_claims=["email": user.email])
        # 5 Return token to the client
        return {"token": token, "user": UserSchema(exclude=["password", "cards"]).dump(user)} # Can also use only=[""] to keep the columns listed
    else:
